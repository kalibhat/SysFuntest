...
#include "cl_types.h" // to be removed sanjeer
...
void DD_IIC_READ_BLDPMP( uint8_t iic_address,uint8_t command,uint16_t* p_data)
{
...
// {
// 	//DD_WRITE_IIC( iic_address,  *data , sizeof(uint16_t));
// 		uint32_t temp =0,temp1=0,retries=0;
...
// 						Cl_SendDatatoconsole(CON_TX_COMMAND_PRINTDATA,&BP_Command_Queue[3].pending,1);*/
// 					uint32_t temp2 =0;
// 					 temp2 = BP_Command_Queue[0].command >> 16 ;
// 					if ( temp2 > 1)
// 					{
// 					BP_Command_Queue[counter+counter1].command = 255;
// 					BP_Command_Queue[counter+counter1].len =0;
